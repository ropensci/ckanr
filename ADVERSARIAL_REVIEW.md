...
- [x] 7. `R/resolve-helpers.R` fallbacks reviewed against membership/collaborator callers (`R/membership.R`, `R/package_collaborators.R`, `R/package_dataset_extras.R` — all pass the result straight into scalar `id`/`group_id`/`org_id`/`username`/`object` params). Confirmed defect: `resolve_group_or_org_id(list(name=...))` returned the whole list (nested JSON → cryptic CKAN 409) while siblings returned `x$name`. Fixed the name fallback for consistency. Also removed byte-identical duplicate `R/aaa-resolve-helpers.R` (no `Collate` field, so build-safe; `R/resolve-helpers.R` is canonical) and fixed stale comment in `R/package_collaborators.R:126` (pointed at `R/membership.R`, actually defined in `R/resolve-helpers.R`). `tests/testthat/test-utils.R`: new offline block covering string/id-list/name-only inputs for all four resolvers. Verified: `test-utils.R` 22 PASS, `test-membership.R` 15 PASS (`NOT_CRAN=true`, live CKAN).
- Next candidate: `tests/testthat/helper-ckanr.R:485-491` source-time side effect (needs maintainer buy-in — deliberately left alone).
- [x] 8. Legacy `related_*` helpers (`R/related_create.R`, `R/related_list.R`, `R/related_show.R`, `R/related_delete.R`): probed live CKAN 2.11.6 — `related_list()` fails with cryptic `400 Bad request - Action name not known`. Fixed with the established `ensure_action_available()` guard (same pattern as 16 uses in `R/admin_ops.R`) as the first statement of all four functions, so users get `The 'related_*' action is unavailable on this CKAN instance`; no behavior change where the action still exists. `Fix #138` intent also verified via `git show 208b71c/d1c2361`: only `ds_search`/`ds_search_sql` examples + devcontainer env, no interaction with `package_search` version gating. New `tests/testthat/test-related.R`: capability-based (passes on any CKAN version). Verified: `test-related.R` 2 PASS (`NOT_CRAN=true`, live CKAN 2.11.6).
- Next candidate: `tests/testthat/helper-ckanr.R:485-491` source-time side effect (needs maintainer buy-in — deliberately left alone).
