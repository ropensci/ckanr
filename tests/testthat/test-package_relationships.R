...
test_that("package_relationship lifecycle works", {
  ...
  rel <- package_relationship_create(pkg_a, pkg_b, relationship_type = "depends_on",
...
test_that("package_relationships_list filters by partner", {
  ...
  package_relationship_create(pkg_a, pkg_b, relationship_type = "derives_from",
  ...
  scoped <- package_relationships_list(pkg_a, id2 = pkg_b,
    relationship_type = "derives_from", url = url, key = key)
  expect_equal(length(scoped), 1)
  expect_equal(scoped[[1]]$object, pkg_b$name)

  package_relationship_delete(pkg_a, pkg_b, relationship_type = "derives_from",
    url = url, key = key)
})
