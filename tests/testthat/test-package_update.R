...
test_that("package_update updates a package", {
  ...
  a <- package_create(name = pkg_name, title = "Original Title", url = url, key = key)
...
test_that("package_update updates multiple fields", {
  ...
  a <- package_create(name = pkg_name, url = url, key = key)
...
test_that("package_update fails well", {
  ...
  # bad key
  expect_error(package_update(list(title = "Test"), "some-id", url = url, key = "invalid-key"),
               "Authorization Error")
})
