...
#' }
package_relationships_list <- function(
  id, id2 = NULL, relationship_type = NULL,
  ...
  res <- ckan_GET(url, "package_relationships_list",
    query = args, key = key,
    opts = list(...)
  )
  switch(as,
...
#' }
package_relationship_create <- function(
...
#' @export
package_relationship_update <- function(
  subject, object, relationship_type,
  comment = NULL, url = get_default_url(), key = get_default_key(),
  as = "list", ...
) {
...
#' @inheritParams package_relationship_create
#' @export
package_relationship_delete <- function(
  subject, object, relationship_type,
